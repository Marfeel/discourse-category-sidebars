...
function redirectToMarfeel(tag) {
    return `https://hub.marfeel.com/?accountId=${tag.replace('accountid-', '')}`;
}
...
export function defaultRenderTag(tag, params) {
  ...
  if (tagName === "a" && !params.noHref) {
    if ((params.isPrivateMessage || params.pmOnly) && User.current()) {
      ...
      path = isAccountIdTag(tag) ? redirectToMarfeel(tag) : `/u/${username}/messages/tags/${tag}`;
    } else {
      path = isAccountIdTag(tag) ? redirectToMarfeel(tag) :`/tag/${tag}`;
    }
  }
